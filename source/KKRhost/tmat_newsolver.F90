  ...
  subroutine allocate_locals_tmat_newsolver(allocmode,irmdnew,lmpot,nspin,vins,aux, &
    ...
    if (allocmode==1) then ! allocate and initialize
      ...
      if (nsra==2) then
        ...
        call memocc(i_stat, i_all, 'VNSPLL', 'allocate_locals_tmat_newsolver')
      else
        i_all = -product(shape(vnspll))*kind(vnspll)
        deallocate (vnspll, stat=i_stat)
        call memocc(i_stat, i_all, 'VNSPLL', 'allocate_locals_tmat_newsolver')
      ...
      call memocc(i_stat, i_all, 'VNSPLL0', 'allocate_locals_tmat_newsolver')
      ...
      call memocc(i_stat, i_all, 'HLK', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(jlk))*kind(jlk)
      deallocate (jlk, stat=i_stat)
      call memocc(i_stat, i_all, 'JLK', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(hlk2))*kind(hlk2)
      deallocate (hlk2, stat=i_stat)
      call memocc(i_stat, i_all, 'HLK2', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(jlk2))*kind(jlk2)
      deallocate (jlk2, stat=i_stat)
      call memocc(i_stat, i_all, 'JLK2', 'allocate_locals_tmat_newsolver')

      i_all = -product(shape(tmatsph))*kind(tmatsph)
      deallocate (tmatsph, stat=i_stat)
      call memocc(i_stat, i_all, 'TMATSPH', 'allocate_locals_tmat_newsolver')

      i_all = -product(shape(ull))*kind(ull)
      deallocate (ull, stat=i_stat)
      call memocc(i_stat, i_all, 'ULL', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(rll))*kind(rll)
      deallocate (rll, stat=i_stat)
      call memocc(i_stat, i_all, 'RLL', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(sll))*kind(sll)
      deallocate (sll, stat=i_stat)
      call memocc(i_stat, i_all, 'SLL', 'allocate_locals_tmat_newsolver')

      if (calculate_left(-1, -1)) then
        i_all = -product(shape(rllleft))*kind(rllleft)
        deallocate (rllleft, stat=i_stat)
        call memocc(i_stat, i_all, 'RLLLEFT', 'allocate_locals_tmat_newsolver')
        i_all = -product(shape(sllleft))*kind(sllleft)
        deallocate (sllleft, stat=i_stat)
        call memocc(i_stat, i_all, 'SLLLEFT', 'allocate_locals_tmat_newsolver')
      else
        i_all = -product(shape(rllleft))*kind(rllleft)
        deallocate (rllleft, stat=i_stat)
        call memocc(i_stat, i_all, 'RLLLEFT', 'allocate_locals_tmat_newsolver')
      ...
      call memocc(i_stat, i_all, 'VINS', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(aux))*kind(aux)
      deallocate (aux, stat=i_stat)
      call memocc(i_stat, i_all, 'AUX', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(ipiv))*kind(ipiv)
      deallocate (ipiv, stat=i_stat)
      call memocc(i_stat, i_all, 'IPIV', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(tmat0))*kind(tmat0)
      deallocate (tmat0, stat=i_stat)
      call memocc(i_stat, i_all, 'TMAT0', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(tmatll))*kind(tmatll)
      deallocate (tmatll, stat=i_stat)
      call memocc(i_stat, i_all, 'TMATLL', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(alpha0))*kind(alpha0)
      deallocate (alpha0, stat=i_stat)
      call memocc(i_stat, i_all, 'ALPHA0', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(dtmatll))*kind(dtmatll)
      deallocate (dtmatll, stat=i_stat)
      call memocc(i_stat, i_all, 'DTMATLL', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(alphall))*kind(alphall)
      deallocate (alphall, stat=i_stat)
      call memocc(i_stat, i_all, 'ALPHALL', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(dalphall))*kind(dalphall)
      deallocate (dalphall, stat=i_stat)
      call memocc(i_stat, i_all, 'DALPHALL', 'allocate_locals_tmat_newsolver')
      i_all = -product(shape(jlk_index))*kind(jlk_index)
      deallocate (jlk_index, stat=i_stat)
      call memocc(i_stat, i_all, 'JLK_INDEX', 'allocate_locals_tmat_newsolver')
