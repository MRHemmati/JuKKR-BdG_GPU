...
contains
  ...
  subroutine tmatimp_newsolver(irm,ksra,lmax,iend,irid,lpot,natyp,ncleb,ipand,irnsd,&
    ...
    call memocc(i_stat, i_all, 'npan_tot', 'tmatimp_newsolver')
    i_all = -product(shape(npan_eq_at))*kind(npan_eq_at)
    deallocate (npan_eq_at, stat=i_stat)
    call memocc(i_stat, i_all, 'npan_eq', 'tmatimp_newsolver')
    i_all = -product(shape(npan_log_at))*kind(npan_log_at)
    deallocate (npan_log_at, stat=i_stat)
    call memocc(i_stat, i_all, 'npan_log_at', 'tmatimp_newsolver')
    i_all = -product(shape(npan_inst))*kind(npan_inst)
    deallocate (npan_inst, stat=i_stat)
    call memocc(i_stat, i_all, 'npan_inst', 'tmatimp_newsolver')
    i_all = -product(shape(jlk_index))*kind(jlk_index)
    deallocate (jlk_index, stat=i_stat)
    call memocc(i_stat, i_all, 'JLK_INDEX', 'tmatimp_newsolver')
    i_all = -product(shape(irmdnew))*kind(irmdnew)
    deallocate (irmdnew, stat=i_stat)
    call memocc(i_stat, i_all, 'irmdnew', 'tmatimp_newsolver')
